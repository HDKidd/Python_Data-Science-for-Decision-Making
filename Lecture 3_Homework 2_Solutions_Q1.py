...
#%% 1.从 yahoo 获取数据； 2.计算日收益率； 3.汇总并输出到 csv 文件

# 创建空数据框
df_pricevol = pd.DataFrame()
...
# 画布创建与设定大小
fig_bp, ax_bp = plt.subplots(figsize=(15, 6))

# 预备作图参数：删除 NaN 行
data_bp = df_logreturn.dropna(0,'all')
...
ax_bp.set_title('Boxplots of Daily Return', pad=20, fontsize=25)            # 标题
ax_bp.set_xlabel('Daily LogReturn', labelpad=10, fontsize=15)               # x轴标记
ax_bp.set_ylabel('Stocks', labelpad=10, fontsize=15)                        # y轴标记
...
# 制作正态分布拟合曲线
fitline = mlab.normpdf(bins, mu, sigma)      # 拟合曲线的函数关系式
...
ax_hist.set_title('Histogram of Daily Return: 600027', pad=20, fontsize=25)  # 标题
ax_hist.set_xlabel('Daily LogReturn', labelpad=10, fontsize=15)              # x轴标记
ax_hist.set_ylabel('Counts', labelpad=10, fontsize=15)                       # y轴标记
...
# 画布创建与设定大小
fig_cand, ax_cand = plt.subplots(figsize=(15, 6))

# 预备作图数据：
df_pricevol['Date'] = df_pricevol.index                         # 从 index 复制日期列
df_pricevol['Date'] = df_pricevol.index.map(mdates.date2num)    # 由于蜡烛图作图时不能引入日期型参数，故先转化为数值
stk = 600027                                                    # 选择股票
data_cand = df_pricevol[['Date', '%s_Open' % stk, '%s_High' % stk, '%s_Low' % stk, '%s_Close' % stk]]    # 获取制图数据
...
ax_cand.set_title('Candlestick of Stock: %s' % stk, pad=20, fontsize=25)      # 标题
ax_cand.set_xlabel('Date', labelpad=10, fontsize=15)                          # x轴标记
ax_cand.set_ylabel('Price (JPY)', labelpad=10, fontsize=15)                   # y轴标记
